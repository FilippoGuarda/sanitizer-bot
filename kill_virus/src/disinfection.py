...
class Disinfection:
    ...
    def __init__(self):
        ...
        # Occupancy
        self.occupancy_subscriber = rospy.Subscriber(
            "/map", OccupancyGrid, self.occupancy_callback)
...
##########################################
        ...
        if self.initFlag:
            ...
            if abs(y_goal-self.currentPose[1]) >= abs(x_goal-self.currentPose[0]):
                goal.target_pose.pose.orientation.z = np.sign(
                    y_goal-self.currentPose[1])*0.7071
...
######### OCCUPANCY ##########
    def occupancy_callback(self, msg):
        self.data = np.asarray(msg.data, dtype=np.int8).reshape(
...
def main():
    ...
    # Read the rooms file
    rooms = open(
        "/home/carlopellettieri/catkin_ws/src/kill_virus/src/rooms.txt", "r")
    for num, room in enumerate(rooms, start=1):
        ...
        prova = obc.greyscale
        resized2 = cv2.flip(cv2.resize(np.copy(prova), dsize=(
            96, 96), interpolation=cv2.INTER_CUBIC), 0)
        resized2[resized2 > 0] = 255
        resized = cv2.flip(prova, 0)
        cv2.imshow("MAP", cv2.resize(np.copy(resized2),
                   (10*resized2.shape[1], 10*resized2.shape[0])))
        ...
        bottom_left1 = [math.floor(
            center1[0] + (x2/cellSize)), math.ceil(center1[1] - (y2/cellSize))]

        sub_map = resized[top_right[1]:bottom_left[1],
                          bottom_left[0]:top_right[0]]
        img = np.copy(sub_map)
        img1 = resized2[top_right1[1]:bottom_left1[1],
                        bottom_left1[0]:top_right1[0]]
        cv2.imshow("Obstacles_map", cv2.resize(
            np.copy(img), (5*img.shape[1], 5*img.shape[0])))
        ...
        cv2.imshow("map resized", cv2.resize(np.copy(img1),
                   (grow*img1.shape[1], grow*img1.shape[0])))
        cv2.waitKey(40)
        img21 = cv2.dilate(resized2, cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (3, 3)), iterations=1)
        img2 = img21[top_right1[1]:bottom_left1[1],
                     bottom_left1[0]:top_right1[0]]

        cv2.imshow("map resized and dilated", cv2.resize(
            np.copy(img2), (grow*img2.shape[1], grow*img2.shape[0])))
        ...
        while ((not rospy.is_shutdown()) and (minEnergy <= 10)):
            ...
            robot_index_y = math.floor(
                matrix_i-1 - math.floor((obc.currentPose[1] - y2)/cellSize))
            ...
            for i in range(0, matrix_i):
                for j in range(0, matrix_j):
                    ...
                    if((abs(x_cell-obc.currentPose[0]) > 0.1 or abs(y_cell-obc.currentPose[1]) > 0.1) and energyUV[i, j] <= 10 and (dist2obs > dist2cell)):
                        energyUV[i, j] = energyUV[i, j] + ((Pl * sampleTime) / (
                            (x_cell - obc.currentPose[0])**2 + (y_cell - obc.currentPose[1])**2))
            ...
            cv2.imshow("map resized and dilated", cv2.resize(
                np.copy(img4), (grow*img4.shape[1], grow*img4.shape[0])))
            ...
            # UV energy plotting
            image_cm = cv2.applyColorMap(
                enel_img.astype(np.uint8), cv2.COLORMAP_JET)
