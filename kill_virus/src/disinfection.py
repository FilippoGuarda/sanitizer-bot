...
########## INITIALIZATION ##########
    def __init__(self):
        ...
        # Occupancy
        self.occupancy_subscriber = rospy.Subscriber("/map", OccupancyGrid , self.occupancy_callback)
        ...
        # Movebase
        self.pub = rospy.Publisher("goal_minimum_energy", PoseWithCovarianceStamped, queue_size=1)
        self.vel_pub = rospy.Publisher('cmd_vel', Twist, queue_size=1)
        self.movebase_subscriber = rospy.Subscriber("goal_minimum_energy", PoseWithCovarianceStamped, self.movebase_client)
...
##########################################
        ...
        if self.initFlag:
            ...
            if abs(y_goal-self.currentPose[1])>=abs(x_goal-self.currentPose[0]):
                goal.target_pose.pose.orientation.z = np.sign(y_goal-self.currentPose[1])*0.7071
                goal.target_pose.pose.orientation.w = 0.7071
            elif abs(x_goal-self.currentPose[0])>abs(y_goal-self.currentPose[1]):
...
########## LASER SCAN #########
    # Here we do a check for obstacles in a cone of 74° in front of him and behind him. And if a obstacle is detected the robot start spin until one of the two are free
    def laser_callback(self, msg):
    	self.laser_scan_values = msg.ranges
    	laser1 = np.asarray(self.laser_scan_values)
    	laser2 = np.concatenate((laser1[0:37], laser1[323:359]), axis=None)
    	laser3 = np.concatenate((laser1[143:179], laser1[180:217]), axis=None)
    	laser4 = np.concatenate((laser1[0:25], laser1[335:359]), axis=None)
    	laser5 = np.concatenate((laser1[155:179], laser1[180:1204]), axis=None)
    	laser = np.concatenate((laser2, laser3), axis=None)
    	if((any(laser2<0.3) or any(laser3<0.3)) and (not self.initFlag)):
...
######### OCCUPANCY ##########    
    def occupancy_callback(self, msg):
        self.data = np.asarray(msg.data, dtype=np.int8).reshape(msg.info.height, msg.info.width)
...
def main():
    ...
    # Read the rooms file
    rooms = open("/home/carlopellettieri/catkin_ws/src/kill_virus/src/rooms.txt", "r")
    for num, room in enumerate(rooms, start=1):
        ...
        prova=obc.greyscale
        resized2 = cv2.flip(cv2.resize(np.copy(prova), dsize=(96, 96), interpolation=cv2.INTER_CUBIC),0)
        resized2[resized2>0]=255
        resized = cv2.flip(prova,0)
        cv2.imshow("MAP",cv2.resize(np.copy(resized2),(10*resized2.shape[1],10*resized2.shape[0])))
        ...
        top_right = [round(center[0] + (4*x1/cellSize)),round(center[1] - (4*y1/cellSize))]
        bottom_left = [round(center[0] + (4*x2/cellSize)),round(center[1] - (4*y2/cellSize))]
        
        top_right1 = [math.ceil(center1[0] + (x1/cellSize)),math.floor(center1[1] - (y1/cellSize))]
        bottom_left1 = [math.floor(center1[0] + (x2/cellSize)),math.ceil(center1[1] - (y2/cellSize))]
        
        sub_map = resized[top_right[1]:bottom_left[1],bottom_left[0]:top_right[0]]
        img = np.copy(sub_map)
        img1 = resized2[top_right1[1]:bottom_left1[1],bottom_left1[0]:top_right1[0]]
        cv2.imshow("Obstacles_map",cv2.resize(np.copy(img),(5*img.shape[1],5*img.shape[0])))
        ...
        cv2.imshow("map resized",cv2.resize(np.copy(img1),(grow*img1.shape[1],grow*img1.shape[0])))
        cv2.waitKey(40)
        img21=cv2.dilate(resized2,cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3,3)),iterations = 1)
        img2=img21[top_right1[1]:bottom_left1[1],bottom_left1[0]:top_right1[0]]
        
        cv2.imshow("map resized and dilated",cv2.resize(np.copy(img2),(grow*img2.shape[1],grow*img2.shape[0])))
        ...
        while ((not rospy.is_shutdown()) and (minEnergy <= 10)): 
            ...
            robot_index_x = math.floor((obc.currentPose[0] - x2)/cellSize)
            robot_index_y = math.floor(matrix_i-1 - math.floor((obc.currentPose[1] - y2)/cellSize))
            ...
            """ 
            iterator_i = np.reshape(np.resize(np.arange(matrix_j),matrix_i*matrix_j), (matrix_i,matrix_j))
            iterator_j = np.reshape(np.resize(np.repeat(np.arange(matrix_i),matrix_j),matrix_i*matrix_j), (matrix_i,matrix_j))

            x_cell = iterator_i * cellSize + x2 
            y_cell = (matrix_i-1 - iterator_j) * cellSize + y2 
            dist_x = abs(x_cell - obc.currentPose[0])
            dist_y = abs(y_cell - obc.currentPose[1])
            ...
            angle_rad_cell = np.arctan2(dist_y, dist_x)
            theta1 = np.degrees(angle_rad_cell)
            ...
            angle_deg_matrix=np.zeros((matrix_i,matrix_j))
                ...
                for j in range(0, matrix_j):
                    ...
                    if ((gamma < 0) and (theta < 0)):
                        if (abs(gamma))>(abs(theta)):
                            angle_deg = int(180+abs(gamma)-abs(theta))
                        else:
                            angle_deg = int(180-abs(theta)+abs(gamma))
                
                    if ((gamma > 0) and (theta < 0)):
                        if (gamma)<(180-abs(theta)):
                            angle_deg = int(180-(abs(theta)+gamma))
                        else:
                            angle_deg = int(360-gamma+180-abs(theta))
                    
                    angle_deg_matrix[i,j] = (angle_deg)
            


            rospy.loginfo("ANGLE DEG MATRIX"+ str((angle_deg_matrix).reshape(-1)))
            laser4 = np.asarray(obc.laser_scan_values)
            dist2obs = np.reshape((laser4[np.ravel((angle_deg_matrix).astype(int))]+0.5),(matrix_i,matrix_j)) 
            dist2cell = np.sqrt(((dist_x)**2)+((dist_y)**2))
            
            #temp1=np.copy(energyUV[(dist_x<=0.1)or(dist_y<=0.1)])
            #temp=np.copy(energyUV[dist_y<=0.1])
            dist_x[robot_index_y,robot_index_x]=100
            #dist_y[robot_index_y,robot_index_x]=100

            #pip, po=np.where((dist2obs > dist2cell)==True)
            #pippo=np.copy(energyUV[pip,po])
            counter=(dist_x==100)
            temp=np.copy(counter)
            rospy.loginfo(temp)
            
            #counter=(dist_x>0.1)
            energyUV[np.logical_not(counter)]= energyUV[np.logical_not(counter)]+((Pl * sampleTime) / (np.square(dist_x[np.logical_not(counter)]) + np.square(dist_y[np.logical_not(counter)])))
            rospy.loginfo(energyUV)
            #energyUV[pip,po]=pippo
            #energyUV[dist_x<=0.1]=temp1
            #energyUV[dist_y<=0.1]=temp2
            energyUV[energyUV>=10]=10
            rospy.loginfo(energyUV)
            """
            ##################################################################################
            ...
            for i in range(0, matrix_i):
                for j in range(0, matrix_j):
                    ...
                    y_cell = (matrix_i-1 - i)*cellSize + y2 + cellSize/2
                    ...
                    if((abs(x_cell-obc.currentPose[0])>0.1 or abs(y_cell-obc.currentPose[1])>0.1) and energyUV[i,j]<=10 and (dist2obs > dist2cell)):
                        energyUV[i,j] = energyUV[i,j] + ((Pl * sampleTime) / ((x_cell - obc.currentPose[0])**2 + (y_cell - obc.currentPose[1])**2))
            ...
            img4= img3+img1
            energyUV_temp=np.copy(energyUV)+np.copy(img2)+np.copy(img3)
            cv2.imshow("map resized and dilated",cv2.resize(np.copy(img4),(grow*img4.shape[1],grow*img4.shape[0])))
            minEnergy = energyUV_temp.min()
            img3[robot_index_y,robot_index_x]=0
            i_min_vector, j_min_vector = np.where(energyUV_temp == minEnergy)
            window=5
            ...
            for pra in i_min_vector: 
            	for it in j_min_vector: 
            		if (((robot_index_x - window < it < robot_index_x + window) and (robot_index_y - window < pra < robot_index_y + window))):
            			imin=pra
            			jmin=it
            ...
            imS = cv2.resize(energyUV, (grow*(matrix_j), grow*(matrix_i)))
            ...
            # UV energy plotting
            image_cm = cv2.applyColorMap(enel_img.astype(np.uint8), cv2.COLORMAP_JET)
